...
class LaunchArgs(
    ...
    private fun getJavaArgs(): List<String> {
        ...
        if (com.movtery.zalithlauncher.setting.AllSettings.forceOpenGl.getValue()) {
            ...
            argsList.add("-Dorg.lwjgl.util.spvc.libname=libshaderconv.so")
            argsList.add("-Dorg.lwjgl.util.shaderc.libname=libshaderc.so")
            argsList.add("-Dorg.lwjgl.util.vma.libname=liblwjgl_vma.so")
        }
